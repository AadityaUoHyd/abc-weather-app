    ...
    >
      {rows.map(({ label, value, Icon }) => (
        <div className="flex justify-between" key={value}>
          ...
          <span>
            <FormatComponent value={value} data={data} />
          </span>
        </div>
      ))}
    </Card>
...
function FormatComponent({ value, data }: { value: string; data: any }) {
  let number: number

  // Map the old current.* structure to the new API structure
  switch (value) {
    case "clouds":
    ...
    case "sunrise":
    case "sunset":
      number = data.sys[value]
      break
    case "uvi":
      // UV index is not available in current weather API, return placeholder
      return "N/A"
    default:
      return "N/A"
  }
